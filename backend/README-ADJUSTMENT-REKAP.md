...
Versi backend: `2026-09-26-published-wrap`. Pembaruan pembaca dokumen dan penghentian rekap harian: lihat `README-PEMBACA-DOKUMEN.md`.
...
4. Periksa URL web app dengan menambahkan `?action=health`. Nilai `backendVersion` harus `2026-09-26-published-wrap`.
...
7. Periksa preview, centang konfirmasi, lalu lanjutkan perhitungan. Tab 5 memisahkan **Lupa Absen dengan Adjustment** dan **Tidak Absen**. Setiap surat lupa absen aktif yang belum dipakai mengoreksi jam menambah satu catatan adjustment, termasuk ketika jam sudah terisi di Excel. Surat yang sudah dipakai di tab 4 diwakili jumlah kejadian koreksinya, tanpa tambahan hitungan file lagi: 1 surat untuk 2 koreksi = 2 adjustment; ditambah 1 surat lain tanpa koreksi = 3 adjustment. Pengiriman ulang request yang sama tidak menambah hitungan.
8. Surat saja tidak mengubah jam, potongan, hari masuk, atau nominal. Setiap datang/pulang yang tetap “-” dihitung **Tidak Absen** dan tetap dikenai 1,25% serta 240 menit; dua kosong = 2,5% serta 480 menit. Batas koreksi pada butir 5 tetap berlaku. Surat tanpa koreksi dihitung pada periode submisi; tanggal kejadian/bulan bagi surat Tukin tidak diterka dari tanggal unggah. Rincian per bulan di tab 5 berlabel **koreksi jam**, bukan jumlah seluruh surat. Menghapus surat membatalkan status hasil perhitungan submisi; proses dan konfirmasi ulang untuk memperbarui angka.
...
- `REKAP_UANG_MAKAN` / `REKAP_TUKIN`: `Hitung_Lupa_Absen` tetap jumlah presensi kosong asli; `Hitung_Adjustment` dan `Hitung_Adjustment_Bulanan` tetap koreksi jam yang benar-benar dilakukan. `Hitung_Tidak_Absen` menyimpan presensi yang masih kosong. Kolom baru `Hitung_Adjustment_Tercatat`, `Hitung_Surat_Lupa_Absen`, `Hitung_Surat_Terpakai`, dan `Hitung_Surat_Tanpa_Koreksi_Jam` menyimpan angka pelaporan surat. Kolom lama tidak dipindah.
- Spreadsheet per pegawai: jam hasil koreksi ditulis pada D/E. Snapshot tab 2 tetap menyimpan jam asli; data koreksi lengkap tetap disimpan terpisah pada sheet internal. Catatan TXT hanya merinci **tanggal yang masih mempunyai TL, PSW, atau Tidak Absen**. Hari normal, flexi yang terpenuhi, Dinas, Cuti, TB, dan Libur tidak ditulis satu per satu. Ringkasan, rumus, sumber tarif, dan peringatan tetap ada. Catatan lama diperbarui dengan ID/link yang sama saat konfirmasi tab 4, bukan diubah otomatis secara massal.

## Rekap Bulanan

Ketentuan terbaru ada di `README-WRAP-BULANAN.md`. Admin memilih bulan/tahun dan SubUnit; publik membaca snapshot yang disimpan dan dipublikasikan admin. Preview memakai `REKAP_UANG_MAKAN` rentang penuh 1–akhir bulan, hasil terkonfirmasi tab 4. Perubahan sumber tidak mengubah publik sebelum disimpan dan diterbitkan lagi. Tidak ada migrasi dari `REKAP_HARIAN`; file per pegawai dan snapshot internal dipertahankan.
...
- Panel evaluasi memisahkan **Tidak Absen** dan **Lupa Absen dengan Adjustment**, termasuk surat tanpa edit jam. Tabel lipat rincian lupa absen dihapus dari admin maupun publik; data backend tetap disimpan. Snapshot publik lama tetap memakai angka yang tersimpan. Untuk memperbaruinya, konfirmasi ulang submisi yang perlu, lalu simpan dan publikasikan snapshot baru melalui admin.
