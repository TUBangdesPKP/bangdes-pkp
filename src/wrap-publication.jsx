...
export function WrapPublication({ endpoint, data, month, loading, onBusy, onSaved }) {
  ...
  const ready = catalog?.version === 1 && Boolean(data?.previewRevision);
  ...
  async function mutate(action, target) {
    ...
    try {
      const payload = action === 'simpan_wrap_bulanan'
        ? { action, month, previewRevision: data.previewRevision, adminKey }
      ...
      onSaved(result.publication);
      if (result.draft) setSelection(result.draft.snapshotId);
      setConfirmation(null);
      setNotice(action === 'simpan_wrap_bulanan'
        ? `Rekap ${monthLabel(month)} sudah disimpan. Halaman publik belum berubah; pilih Tampilkan ke Publik untuk menerbitkannya.`
  ...
  return <section aria-label="Pengaturan publikasi rekap" className="my-5 rounded-2xl border border-slate-200 bg-white p-5 space-y-4 text-sm">
    ...
    <div className="grid md:grid-cols-2 gap-5">
      <div className="space-y-3">
        ...
        <button disabled={!ready || loading || Boolean(busy) || !adminKey || !data?.employees?.length} onClick={() => mutate('simpan_wrap_bulanan')} className="inline-flex items-center gap-2 rounded-lg bg-[#0E5B73] text-white px-4 py-2.5 font-semibold disabled:opacity-40"><Save size={16}/>{busy === 'simpan_wrap_bulanan' ? 'Menyimpan…' : `Simpan Rekap ${monthLabel(month)}`}</button>
        <p className="text-xs text-slate-500">Menyimpan ulang membuat versi baru jika data berubah. Rekap publik tetap pada versi sebelumnya sampai dipublikasikan lagi.</p>
      </div>
      <div className="space-y-3">
        ...
        <button disabled={!draft || active || Boolean(busy) || !adminKey} onClick={() => setConfirmation(draft)} className="inline-flex items-center gap-2 rounded-lg border border-[#0E5B73] text-[#0E5B73] px-4 py-2.5 font-semibold disabled:opacity-40"><Globe size={16}/>{active ? 'Sudah Tampil di Publik' : 'Tampilkan ke Publik'}</button>
