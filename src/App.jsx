...
const UserDashboardView = ({ loggedInUser, onLogoutRequest, navigate, currentView, activeStep }) => {
  ...
  const handleFileChange = async (e) => {
    if (e.target.files && e.target.files[0]) {
      ...
      const checkExisting = async (nip, modul, identifier) => {
        ...
        try {
          ...
          if (res.ok) {
            const json = await res.json();
            if (json.exists) {
              localStorage.setItem(cacheKey, 'true'); // set cache lokal
              return true;
            }
          }
        } catch (err) {
      ...
      if (activeTab === 'spt') {
         await new Promise(resolve => setTimeout(resolve, 800)); // Simulasi kecil agar UI parsing terlihat
         const expectedNip = loggedInUser.NIP;
         setParsedData({
           isValid: true,
           nip: expectedNip,
      ...
      try {
        const result = await parseDocumentPresensi(file, selectedPeriod); // Pass selectedPeriod for dynamic folder
                  ...
                  <div className="lg:col-span-8 space-y-4">
                    ...
                    <div className="bg-white rounded-3xl border border-gray-200 shadow-xs p-6">
                      <div className="mb-4 pb-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div>
                          <h3 className="text-sm font-black text-gray-900">Preview {activeTab === 'spt' ? 'Data Surat Tugas' : 'Data Presensi'}</h3>
                        ...
                        <div className="text-left sm:text-right text-[11px] bg-[#F8FAFC] px-4 py-2.5 rounded-2xl border border-gray-200 space-y-0.5 min-w-[200px]">
                          ...
                          <p className="text-[10px] text-teal-700 font-extrabold">Periode: {parsedData ? parsedData.periode : (activeTab === 'spt' ? 'Semua Periode' : (selectedPeriod?.periodeEvent || '-'))}</p>
                        </div>
                      </div>
                      
                      {activeTab === 'spt' ? (
                        <div className="overflow-auto border border-gray-200 rounded-2xl bg-gray-50 flex items-center justify-center p-16 shadow-inner relative min-h-[450px]">
                            {selectedFile ? (
                                <div className="text-center animate-in fade-in zoom-in duration-300">
                                   <FileText size={48} className="mx-auto text-teal-700 mb-3" />
                                   <h4 className="font-bold text-gray-800 text-sm">{selectedFile.name}</h4>
                                   <p className="text-xs text-gray-500 mt-1">Dokumen Siap Diupload ke Folder Arsip</p>
                                </div>
                            ) : (
                                <div className="text-center opacity-60">
                                   <FileText size={48} className="mx-auto text-gray-400 mb-3" />
                                   <h4 className="font-bold text-gray-500 text-sm">Pratinjau Kosong</h4>
                                </div>
                            )}
                        </div>
                      ) : (
