...
const extractSptData = (lines) => {
  const nipSet = new Set();
  let extractedDate = '-';
  
  // Toleransi OCR: Titik dua (:) terkadang hilang atau terbaca titik koma (;)
  const waktuRegex = /Waktu\s*[:;]?\s*(.*)/i;
  const periodeRegex = /Periode\s*Perjalanan\s*Dinas[\s\|:;]*([^\|]+)/i;
...
    // 1. Ekstrak Tanggal
    if (extractedDate === '-') {
        const waktuMatch = line.match(waktuRegex);
        if (waktuMatch) {
          extractedDate = waktuMatch[1].replace(/[:;]/g, '').trim();
        } else {
          const periodeMatch = line.match(periodeRegex);
          if (periodeMatch) {
...
    // 2. Ekstrak NIP
    // Karena dokumen OCR ttd basah sering kali menyisipkan spasi berlebih pada angka,
    // kita hapus seluruh spasi di baris ini untuk menyatukan kembali struktur 18 digit NIP.
    const cleanLine = line.replace(/\s+/g, '');
    const nipMatches = cleanLine.matchAll(/(19\d{16}|20\d{16})/g);
    
    for (const match of nipMatches) {
        const nipStr = match[1];
        // Abaikan NIP Direktur/Pejabat penandatangan (contoh: Ibu Rini)
        if (nipStr === '197012151998032007') continue; 
        nipSet.add(nipStr);
    }
  }
  
  // 3. Lookup Nama ke Cache Database
  let dbPegawai = [];
  const cached = localStorage.getItem('cached_pegawai_json');
  if (cached) {
    try {
      dbPegawai = JSON.parse(cached);
    } catch (e) {
      console.error("Gagal membaca cache data pegawai:", e);
    }
  }

  const pegawaiList = [];
  nipSet.forEach((nip) => {
      let nama = 'Pegawai (Belum terdaftar di Bank Data)';
      const found = dbPegawai.find(p => String(p.NIP).trim() === nip);
      if (found && found.Nama) {
          nama = found.Nama;
      }
      pegawaiList.push({ nama, nip });
  });
...
const parseDocumentPresensi = async (file, selectedPeriod = null, activeTab = null) => {
...
  if (activeTab === 'spt') {
      const sptData = extractSptData(lines);
      return {
          isValid: true,
          isSpt: true,
...
          }
        } catch (err) {
          console.warn("Gagal mengecek status ke backend, menggunakan fallback lokal.", err);
        }
        return false;
      };

      try {
        const result = await parseDocumentPresensi(file, selectedPeriod, activeTab); 
...
                            className="w-full py-3.5 mt-2 rounded-2xl bg-teal-50 border border-teal-200 text-teal-800 font-bold text-xs shadow-sm transition-all flex items-center justify-center gap-2 hover:bg-teal-100 cursor-pointer animate-in fade-in slide-in-from-top-2 duration-300"
                          >
...
                                            {selectedFile && !isParsing ? 'Tidak ada NIP pegawai ditemukan di berkas.' : 'Belum Ada Berkas Pratinjau'}
                                          </p>
                                          <p className="text-xs">
                                            {selectedFile && !isParsing ? 'Sistem hanya mendeteksi NIP (18-digit) untuk menampilkan data pegawai.' : 'Silakan unggah dokumen PDF Surat Tugas Anda.'}
                                          </p>
                                        </div>
                                      </div>
                                    </td>
