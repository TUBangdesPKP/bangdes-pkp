...
window.fetch = async (url,options) => {
  ...
  window.dispatchEvent(new CustomEvent('wrap-request',{detail:`${++count}: ${JSON.stringify(payload)}`}));
  ...
  return Response.json({status:'success',wrapVersion:2,month,months:['2026-08','2025-12'],units:['Subbagian Tata Usaha','Subdirektorat Uji'],coverage:{submitted:rows.length,available:rows.length},updatedAt:new Date().toISOString(),employees:rows,daily:rows.flatMap(person=>Array.from({length:6},(_,i)=>({nip:person.nip,tanggal:`${month}-${String(3+i).padStart(2,'0')}`,status:'WFO',libur:false,hadir:true,arrival:person.avgArrival}))),documents:rows.length?[{nip:'2',type:'spt',id:'test',tujuan:'Kota Uji'}]:[]});
};
