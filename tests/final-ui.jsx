...
const rows = [
  {tanggal:'2026-08-20',hari:'Kamis',datang:'-',pulang:'-',keteranganAwal:'WFO',keterangan:'WFO',libur:false,konflik:false,dokumen:[]},
...
window.fetch = async (url, options) => {
  ...
  const calculation={modul:context.modul,complete:true,warnings:[],jabatan:'Analis Perumahan',sources:{tarif:'Data_Pegawai, baris uji, kolom Besaran Uang Makan'},
    totals:{masuk:1,hariKerja:4,dinas:2,cuti:1,tb:0,libur:1,flexi:0,terlambat:1,psw:1,tidakMasuk:0,menitTelat:15,menitPsw:31,menitTanpaPresensi:240,totalMenit:286,potonganAbsensi:1.25,lupaAbsen:2,adjusted:1,unadjusted:1,adjustmentReported:3,adjustmentDocuments:3,adjustmentDocumentsUsed:1,adjustmentDocumentsUnclaimed:2,adjustmentMonths:{'2026-08':1,'2026-09':0}},
  ...
  return new Response(JSON.stringify({status:'success',...context,calculation,adjustments:payload.adjustments || {},adjustmentDocuments:[{fileId:'letter',fileName:'Surat Lupa Absen Uji.pdf'}],note:{url:'#mock-note'},spreadsheetId:'test-only',spreadsheetUrl:'#mock-spreadsheet',revision:'test',rows: rows.map(row => ({...row,jamKerja:payload.schedules?.[row.tanggal]||'biasa',keterangan:payload.resolutions?.[row.tanggal] || row.keterangan}))}));
};
function Fixture() {
